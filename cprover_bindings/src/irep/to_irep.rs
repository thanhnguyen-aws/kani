    ...
    fn to_irep(&self, mm: &MachineModel) -> Irep {
        match self {
            ...
            StmtBody::Goto { dest, loop_invariants, loop_assigns } => {
                ...
                let linv = loop_invariants.clone().map(|inv| inv.clone().and(Expr::bool_true()).to_irep(mm));
                let lassigns = loop_assigns.clone().map(|assigns| {
                ...
                let stmt_goto = code_irep(IrepId::Goto, vec![])
                    ...
                    .with_named_sub_option(IrepId::CSpecLoopInvariant, linv)
                    .with_named_sub_option(IrepId::CSpecAssigns, lassigns);
                if loop_assigns.is_some(){
                    println!("stmt_goto : {:?}", stmt_goto);
                }
                stmt_goto
            }
